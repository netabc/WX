// pages/testbluetooth/myBluetooth.js
var app = getApp();
...
Page({
   ...
   */
  data: {
    url_save: "http://hytest.chobmi.cn/PhoneTest/Controller/testUp",
    url_get: "http://hytest.chobmi.cn/PhoneTest/Controller/testDown",
    fialtry: 0,
    recv_value:'',
    send_string: true, 
    recv_string: true 
  ...
  onReady: function () {
    var that = this;
    app.globalData.myCallback = function (available) {
      that.setData({ available: available });
    }
    that.found();
  },
        ...
        success: function (res) {
          ...
          wx.getBLEDeviceServices({
            ...
            success: function (service) {
              ...
              that.getOutBluetooth();     //调用获取特征值函数
            },fail:function(){
              wx.hideLoading();
            }
          });
  ...
  group: function (data, key) {
    ...
    console.log("group" + JSON.stringify(that.data));
  },
  /* item点击事件 */
  onLianTap: function (event) {
    ...
    console.log("onLianTap" + JSON.stringify(event));
  ...
  notycharacteristicsId: function () {
    ...
    var string_value = "";
    var recve_value = "";
    console.log("dealBlueData="+JSON.stringify(that.data));
  ...
  sendData: function (event) {
    ...
    var string_value = "";
    var recve_value = "";
    console.log("sendData" + JSON.stringify(that.data));
    ...
    if (!that.data.deviceId || !that.data.serviceId){
      wx.showToast({
  ...
  sendServerId:function(event){
    this.setData({ server_id: event.detail.value});
  },
  sendServer: function () {
    var user_id = this.data.server_id;
    var value = this.data.recv_value;
    console.log("userid:"+user_id+">>value:"+value);
    if (!user_id || !value) {
      wx.showToast({
        title: '请输入正确的值',
        success: function (res) { },
    ...
    var url = this.data.url_save;
    wx.request({
      url: url+ "?user_id=" + user_id + "&value=" + value,
      method: "GET",
      header: {
  ...
  readServer: function () {
    ...
    var url = this.data.url_get;
    wx.request({
      url: url + "?time=" + new Date() + "&user_id=" + user_id,
      method: "GET",
      header: {
        'Content-Type': "application/json",
      },
      success: function (res) {
        var data = ""+res.data;
        that.setData({ recv_server_value: data });
        console.log(data);
      },
      fail: function (res) {
        wx.showModal({
  ...
  sendServerData: function (e) {
    this.setData({
      recv_value: e.detail.value
    });
  }
})